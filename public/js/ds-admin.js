...
function _openUser(uid) {
  ...
  // Historique connexion (simulé depuis loginHistory si présent)
  ...
  _showModal(`
    <div class="adm-modal-head">
      <div class="adm-card-title" style="margin-bottom:0;">
        <i class="fa-solid fa-user-circle"></i> Profil utilisateur
      </div>
      <button class="adm-modal-close" onclick="window.DS_ADMIN?._removeModal()">
        <i class="fa-solid fa-xmark"></i>
      </button>
    ...
    <div class="adm-modal-body">
      ...
      <!-- Avatar + identité -->
      <div style="display:flex;align-items:center;gap:16px;margin-bottom:20px;padding-bottom:18px;border-bottom:1px solid rgba(255,255,255,.06);">
        <div class="adm-avatar-lg" style="background:linear-gradient(135deg,${planColor}22,${planColor}44);color:${planColor};">
          ${u.photoURL ? `<img src="${u.photoURL}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.style.display='none'">` : initials}
        </div>
        <div style="flex:1;min-width:0;">
          <div style="font-family:'Syne',sans-serif;font-size:16px;font-weight:900;">${u.prenom||''} ${u.nom||''}</div>
          <div style="font-size:10px;color:${A.muted};margin-top:2px;overflow:hidden;text-overflow:ellipsis;">${u.email||'—'}</div>
      ...
      <!-- Infos entreprise -->
      <div class="adm-info-grid" style="margin-bottom:16px;">
      ...
      <div style="margin-bottom:16px;">
        <div class="adm-card-title"><i class="fa-solid fa-clock-rotate-left"></i>Historique de connexion</div>
        ${loginHistory.length > 0 ? `
          <div style="display:flex;flex-direction:column;gap:5px;max-height:120px;overflow-y:auto;">
            ${loginHistory.slice(-6).reverse().map(l => `
              <div style="display:flex;align-items:center;gap:10px;padding:6px 10px;background:rgba(255,255,255,.02);border-radius:7px;">
                <i class="fa-solid fa-arrow-right-to-bracket" style="font-size:9px;color:${A.ice};"></i>
                <span style="font-family:'JetBrains Mono',monospace;font-size:10px;flex:1;">${_dateShort(_ts(l.ts))}</span>
                <span style="font-size:9px;color:${A.muted};">${l.ip || '—'}</span>
                <span style="font-size:9px;color:${A.muted};">${l.device || '—'}</span>
              </div>
            `).join('')}
          </div>
        ` : `
          <div style="font-size:11px;color:${A.muted};padding:8px 0;">
      ...
      <div style="margin-bottom:18px;">
        <div class="adm-card-title"><i class="fa-solid fa-microscope"></i>Analyses (${analyses.length})</div>
        ${analyses.length ? `
          <div style="display:flex;flex-direction:column;gap:5px;max-height:160px;overflow-y:auto;">
            ${analyses.slice(0, 10).map(a => {
              const c = a.score>=75 ? A.emerald : a.score>=50 ? A.amber : a.score>=25 ? '#f97316' : A.ruby;
              return `
                <div style="display:flex;align-items:center;justify-content:space-between;
                  padding:8px 12px;background:rgba(255,255,255,.03);border-radius:8px;
                  border:1px solid rgba(255,255,255,.05);gap:10px;">
                  <span style="font-size:11px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${a.entreprise||'—'}</span>
                  <span style="font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:700;color:${c};">${a.score}/100</span>
                  <span style="font-size:9px;color:${A.muted};">${_dateShort(_ts(a.createdAt))}</span>
                </div>
              `;
            }).join('')}
          </div>
        ` : `<div style="font-size:11px;color:${A.muted};padding:8px 0;">Aucune analyse effectuée</div>`}
      </div>

      <!-- Actions -->
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <button class="adm-btn-primary" style="flex:1;justify-content:center;min-width:120px;"
          onclick="window.DS_ADMIN?._removeModal();window.DS_ADMIN?.openChangePlan('${uid}','${plan}')">
          <i class="fa-solid fa-credit-card"></i> Changer le plan
        </button>
        <button class="adm-btn-secondary" style="justify-content:center;"
          onclick="window.DS_ADMIN?.sendResetPwd('${uid}','${u.email}')">
          <i class="fa-solid fa-key"></i> Reset mot de passe
        </button>
        <button class="adm-btn-secondary warn" style="justify-content:center;"
        ...
        <button class="adm-btn-danger" style="justify-content:center;"
          onclick="window.DS_ADMIN?._removeModal();window.DS_ADMIN?.confirmDelete('${uid}','${u.email}')">
          <i class="fa-solid fa-trash"></i>
        </button>
      </div>
