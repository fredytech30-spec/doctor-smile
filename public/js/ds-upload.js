...
window.DEMO_DATA = {
  ...
  probabiliteDefaut:12.4, confidence:94, model:'RF + XGBoost + LightGBM',
  auc:.92, createdAt:new Date(), processingMs:340,
  scoreHistory:[58,62,59,65,68,70,72],
  shapValues:[
    { feature:'Ratio de liquidité',     value:+8.2, pct:82, direction:'positive' },
    { feature:'Marge nette',            value:+5.4, pct:54, direction:'positive' },
    { feature:'Délai paiement clients', value:-6.1, pct:61, direction:'negative' },
    { feature:"Ratio d'endettement",    value:-4.8, pct:48, direction:'negative' },
    { feature:'Croissance CA',          value:+3.2, pct:32, direction:'positive' },
  ],
  ratios:[
    { name:'Liquidité générale',  value:1.42, unit:'',  benchmark:'> 1.5', status:'yellow', score:71 },
    { name:'Marge nette',         value:8.3,  unit:'%', benchmark:'> 10%', status:'yellow', score:63 },
    { name:'ROE',                 value:12.1, unit:'%', benchmark:'> 15%', status:'yellow', score:61 },
    { name:"Ratio d'endettement", value:.68,  unit:'',  benchmark:'< 0.5', status:'red',    score:45 },
    { name:'Rotation actifs',     value:1.83, unit:'',  benchmark:'> 1.5', status:'green',  score:85 },
    { name:'BFR / CA',            value:24.6, unit:'%', benchmark:'< 20%', status:'orange', score:52 },
  ],
  radarDimensions:[
    { label:'Liquidité',   value:71 }, { label:'Rentabilité', value:63 },
    { label:'Solvabilité', value:55 }, { label:'Activité',    value:85 },
    { label:'Croissance',  value:68 }, { label:'Structure',   value:58 },
  ],
  recommendations:[
    { level:'high',   title:'Réduire le délai client',          description:'Délai 68j vs norme 45j. Relance → ~120k€ libérés.',        icon:'fa-exclamation' },
    { level:'medium', title:'Optimiser la structure financière', description:"Endettement 0.68 > 0.5. Refinancement LT conseillé.",       icon:'fa-chart-line'  },
    { level:'low',    title:'Levier de croissance identifié',    description:'Rotation actifs excellente (1.83). Investissement capacité.', icon:'fa-seedling'   },
  ],
};
...
window.DS_UPLOAD = {
  ...
  launchML() { this.closeModalDirect(); this.sendToAPI(); },
  ...
  startPipelineUI() {
    ...
    stps.innerHTML=this.ML_STEPS.map((s,i)=>
    ...
    S.pipelineTimer=setInterval(()=>{
      if(S.pipelineStep>=this.ML_STEPS.length-1) return;
      if(S.pipelineStep>0){
      ...
      S.pipelineStep++;
      const p=Math.round((S.pipelineStep/this.ML_STEPS.length)*88);
  ...
  async sendToAPI() {
    ...
    this.startPipelineUI();
    try {
      // Import dynamique fetchWithAuth depuis utils.js
      ...
      if (res.analyseId) {
        ...
        showToast('⚡ Pipeline lancé — résultat dans quelques secondes…','info');
      }
    } catch(err) {
      this.stopPipelineUI();
  ...
  async uploadPDF(file) {
    ...
    form.append('file',file); form.append('userId',S.user.uid);
    ...
    } catch { this.stopPipelineUI(); showToast('Erreur upload PDF','err'); }
  },
