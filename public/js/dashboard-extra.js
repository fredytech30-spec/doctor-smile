...
function render3DGauge(cid,prob,zone){
  var T=window.THREE;
  if(!T){setTimeout(function(){render3DGauge(cid,prob,zone);},400);return;}
  var el=_cleanEl(cid);if(!el)return;
  var W=el.clientWidth||260,H=el.clientHeight||130;
  if(W<10){setTimeout(function(){render3DGauge(cid,prob,zone);},250);return;}
  var sc=new T.Scene(),cam=new T.PerspectiveCamera(50,W/H,.1,100);
  var ren=new T.WebGLRenderer({antialias:true,alpha:true});
  ren.setSize(W,H);ren.setClearColor(0,0);el.appendChild(ren.domElement);
  cam.position.set(0,.5,3.2);cam.lookAt(0,0,0);
  sc.add(new T.AmbientLight(0xffffff,.5));
  var dl=new T.DirectionalLight(0x7DD3FC,1.5);dl.position.set(3,5,3);sc.add(dl);
  var ZC={saine:0x10b981,vigilance:0xf59e0b,risque:0xf97316,critique:0xef4444};
  var col=ZC[zone]||0xf59e0b;
  function arc(r,tb,a0,a1,color){
    var pts=[];for(var i=0;i<=80;i++){var a=a0+(a1-a0)*(i/80);pts.push(new T.Vector3(Math.cos(a)*r,Math.sin(a)*r,0));}
    var g=new T.TubeGeometry(new T.CatmullRomCurve3(pts),80,tb,10,false);
    return new T.Mesh(g,new T.MeshPhongMaterial({color:color,shininess:120}));
  }
  sc.add(arc(1.2,.10,Math.PI,Math.PI*2,0x1e293b));
  sc.add(arc(1.2,.10,Math.PI,Math.PI+(prob/100)*Math.PI,col));
  var sph=new T.Mesh(new T.SphereGeometry(.28,32,32),new T.MeshPhongMaterial({color:col,shininess:200,transparent:true,opacity:.92}));
  sc.add(sph);
  var parts=[];for(var i=0;i<8;i++){
    var p=new T.Mesh(new T.SphereGeometry(.022,8,8),new T.MeshBasicMaterial({color:col,transparent:true,opacity:.6}));
    var a=Math.random()*Math.PI*2,r=.45+Math.random()*.45;
    p.position.set(Math.cos(a)*r,Math.sin(a)*r-.25,(Math.random()-.5)*.3);
    p._sp=.012+Math.random()*.018;p._of=Math.random()*Math.PI*2;
    sc.add(p);parts.push(p);
  }
  var fr,t=0;
  function anim(){
    fr=requestAnimationFrame(anim);t+=.016;sph.rotation.y+=.015;
    for(var i=0;i<parts.length;i++){parts[i].position.y+=Math.sin(t*parts[i]._sp*3+parts[i]._of)*.002;parts[i].material.opacity=.4+Math.sin(t*2+parts[i]._of)*.2;}
    ren.render(sc,cam);
  }
  anim();
  el._dsCleanup=function(){cancelAnimationFrame(fr);ren.dispose();};
  _chatBtn(el,'La jauge 3D montre '+prob+'% de probabilité de faillite en zone '+zone+'. Explique ce niveau de risque et les actions recommandées.');
}
...
function renderScoreDonut(cid,score,zone){
  var C=window.Chart;if(!C){setTimeout(function(){renderScoreDonut(cid,score,zone);},400);return;}
  var wrap=document.getElementById(cid);if(!wrap)return;
  _destroyChart('c_'+cid);
  wrap.innerHTML='<canvas id="c_'+cid+'" style="width:100%;height:100%;"></canvas>';
  var canvas=document.getElementById('c_'+cid);
  var ZC={saine:'#10b981',vigilance:'#f59e0b',risque:'#f97316',critique:'#ef4444'};
  var col=ZC[zone]||'#f59e0b';
  var ch=new C(canvas,{
    type:'doughnut',
    data:{datasets:[{data:[score,100-score],backgroundColor:[col,'rgba(255,255,255,.06)'],borderWidth:0,hoverOffset:4}]},
    options:{responsive:true,maintainAspectRatio:false,cutout:'74%',plugins:{legend:{display:false},tooltip:{enabled:false}},animation:{animateRotate:true,duration:900}}
  });
  _saveChart('c_'+cid,ch);
  var wrapPar=wrap.parentElement;
  if(wrapPar){
    var ex=document.getElementById('_dl_'+cid);if(ex)ex.remove();
    var dl=document.createElement('div');dl.id='_dl_'+cid;
    dl.style.cssText='position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center;pointer-events:none;';
    dl.innerHTML='<div style="font-family:Syne,sans-serif;font-size:26px;font-weight:900;color:'+col+';">'+score+'</div>'
      +'<div style="font-size:8px;color:rgba(255,255,255,.3);">/100</div>';
    wrapPar.appendChild(dl);
  }
  _chatBtn(wrap,'Score Doctor Score de '+score+'/100 en zone '+zone+'. Explique ce score en détail, son interprétation et les leviers d\'amélioration.');
}

function renderRadarChart(cid,radarDims){
  var C=window.Chart;if(!C){setTimeout(function(){renderRadarChart(cid,radarDims);},400);return;}
  var el=document.getElementById(cid);if(!el)return;
  _destroyChart('c_'+cid);
  if(!radarDims||!radarDims.length){el.innerHTML='<div style="padding:20px;text-align:center;font-size:10px;color:rgba(255,255,255,.25);">Données radar manquantes</div>';return;}
  ...
  var ch=new C(canvas,{
    type:'radar',
    data:{
      labels:radarDims.map(function(d){return d.l||d.label||'';}),
      datasets:[
        {label:'Votre profil',data:radarDims.map(function(d){return d.v||d.value||0;}),borderColor:'rgba(125,211,252,.9)',backgroundColor:'rgba(125,211,252,.1)',borderWidth:2,pointBackgroundColor:'#7DD3FC',pointRadius:4,pointHoverRadius:6},
        {label:'Référence',data:radarDims.map(function(){return 70;}),borderColor:'rgba(255,215,0,.35)',backgroundColor:'rgba(255,215,0,.04)',borderWidth:1,borderDash:[4,4],pointRadius:0}
      ]
    },
    options:{
      responsive:true,maintainAspectRatio:false,
      plugins:{legend:{labels:{color:'rgba(255,255,255,.38)',font:{family:'Syne',size:9}}}},
      scales:{r:{grid:{color:'rgba(255,255,255,.06)'},angleLines:{color:'rgba(255,255,255,.06)'},pointLabels:{color:'rgba(255,255,255,.5)',font:{family:'Syne',size:9}},ticks:{display:false},suggestedMin:0,suggestedMax:100}}
    }
  });
  _saveChart('c_'+cid,ch);
  _chatBtn(el,'Radar : '+radarDims.map(function(d){return d.l+'='+d.v;}).join(', ')+'. Compare au benchmark 70/100. Quelles dimensions améliorer ?');
}
...
function renderTimelineFixed(data,svgId){
  svgId=svgId||'tl-svg';
  var svg=document.getElementById(svgId);if(!svg)return;
  if(!data||!data.length){svg.innerHTML='<text x="280" y="52" text-anchor="middle" font-family="Syne,sans-serif" font-size="11" fill="rgba(255,255,255,.2)">Historique insuffisant (1 point)</text>';return;}

  var par=svg.parentElement;
  if(par){par.style.overflow='hidden';par.style.position='relative';}
  svg.style.cssText='width:100%;height:100%;display:block;overflow:hidden;';
  svg.setAttribute('viewBox','0 0 560 105');
  svg.setAttribute('preserveAspectRatio','xMidYMid meet');

  var TW=560,TH=105,pad=28;
  var mn=Math.min.apply(null,data)-8,mx=Math.max.apply(null,data)+8;
  if(mn===mx){mn=Math.max(0,mn-15);mx=Math.min(100,mx+15);}

  var pts=data.map(function(v,i){
    return {x:pad+(i/Math.max(data.length-1,1))*(TW-pad*2),y:TH-pad-((v-mn)/(mx-mn))*(TH-pad*2)};
  });

  // Courbe de Bézier lissée
  var lineParts=[];
  for(var i=0;i<pts.length;i++){
    if(i===0){lineParts.push('M'+pts[0].x.toFixed(1)+','+pts[0].y.toFixed(1));}
    else{var cpx=(pts[i-1].x+pts[i].x)/2;lineParts.push('C'+cpx.toFixed(1)+','+pts[i-1].y.toFixed(1)+' '+cpx.toFixed(1)+','+pts[i].y.toFixed(1)+' '+pts[i].x.toFixed(1)+','+pts[i].y.toFixed(1));}
  }
  var pd=lineParts.join(' ');
  var adParts='M'+pts[0].x.toFixed(1)+','+TH+' ';
  for(var i=0;i<pts.length;i++){adParts+='L'+pts[i].x.toFixed(1)+','+pts[i].y.toFixed(1)+' ';}
  adParts+='L'+pts[pts.length-1].x.toFixed(1)+','+TH+' Z';

  var MONTHS=['Jan','Fév','Mar','Avr','Mai','Jun','Jul','Aoû','Sep','Oct','Nov','Déc'];
  var now=new Date().getMonth();
  var labs=data.map(function(_,i){return MONTHS[(now-data.length+1+i+12)%12];});
  var uid='tl_'+svgId;

  var circles='';
  for(var i=0;i<pts.length;i++){
    var isLast=i===pts.length-1;
    var fill=isLast?'#FFD700':'#7DD3FC';
    var r2=isLast?5.5:3.5;
    var trend=i>0?(data[i]>data[i-1]?'↑':data[i]<data[i-1]?'↓':'→'):'';
    var tc=trend==='↑'?'#10b981':trend==='↓'?'#ef4444':'#f59e0b';
    circles+='<circle cx="'+pts[i].x.toFixed(1)+'" cy="'+pts[i].y.toFixed(1)+'" r="'+r2+'" fill="'+fill+'" stroke="rgba(2,4,11,.9)" stroke-width="2"/>'
      +(isLast?'<circle cx="'+pts[i].x.toFixed(1)+'" cy="'+pts[i].y.toFixed(1)+'" r="9" fill="none" stroke="'+fill+'44" stroke-width="2"/>':'')
      +'<text x="'+pts[i].x.toFixed(1)+'" y="'+(pts[i].y-11).toFixed(1)+'" text-anchor="middle" font-family="Syne,sans-serif" font-size="9" font-weight="700" fill="rgba(255,255,255,.62)">'+data[i]+'</text>'
      +(trend?'<text x="'+(pts[i].x+9).toFixed(1)+'" y="'+(pts[i].y-7).toFixed(1)+'" font-family="Syne,sans-serif" font-size="9" fill="'+tc+'">'+trend+'</text>':'')
      +'<text x="'+pts[i].x.toFixed(1)+'" y="'+(TH-7).toFixed(1)+'" text-anchor="middle" font-family="Syne,sans-serif" font-size="7.5" fill="rgba(255,255,255,.28)">'+labs[i].toUpperCase()+'</text>';
  }

  svg.innerHTML='<defs>'
    +'<linearGradient id="g'+uid+'" x1="0" y1="0" x2="0" y2="1">'
    +'<stop offset="0%" stop-color="rgba(125,211,252,.22)"/><stop offset="100%" stop-color="rgba(125,211,252,0)"/></linearGradient>'
    +'<clipPath id="cp'+uid+'"><rect width="'+TW+'" height="'+TH+'"/></clipPath></defs>'
    +'<g clip-path="url(#cp'+uid+')">'
    +'<path d="'+adParts+'" fill="url(#g'+uid+')"/>'
    +'<path d="'+pd+'" fill="none" stroke="rgba(125,211,252,.82)" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>'
    +circles+'</g>';
}
...
window.DS_EXTRA={
  render3DGauge:render3DGauge,render3DGlobe:render3DGlobe,render3DBarChart:render3DBarChart,
  renderScoreDonut:renderScoreDonut,renderRadarChart:renderRadarChart,
  renderWaterfallChart:renderWaterfallChart,renderBulletRatios:renderBulletRatios,
  renderHeatmap:renderHeatmap,renderTimelineFixed:renderTimelineFixed,
