// ════════════════════════════════════════════════════════════════
//  payment.js — Doctor Smile
//  Gestion des paiements via NotchPay et Fapshi côté frontend
//  UX/UI Professionnelle avec confirmation et gestion d'erreurs
// ════════════════════════════════════════════════════════════════
...
// ── Modal de confirmation détaillée avant paiement ─────────────────
async function showPaymentConfirmation(plan, operatorName) {
  return new Promise((resolve) => {
    const modal = document.createElement('div');
    modal.style.cssText = `
      position:fixed;inset:0;z-index:10000;
      display:flex;align-items:center;justify-content:center;
      background:rgba(2,4,11,.9);backdrop-filter:blur(16px);
      animation:mIn .3s ease;
    `;
    
    modal.innerHTML = `
      <div style="
        background:rgba(10,14,26,.98);border:1px solid rgba(139,127,240,.2);
        border-radius:24px;padding:40px;max-width:480px;width:90%;
        box-shadow:0 40px 100px rgba(0,0,0,.7);
      ">
        <div style="text-align:center;margin-bottom:32px;">
          <div style="font-family:'Syne',sans-serif;font-size:24px;font-weight:900;color:#fff;margin-bottom:8px;">
            Confirmer le paiement
          </div>
          <div style="font-size:12px;color:rgba(255,255,255,.5);">
            Vous allez être redirigé vers ${operatorName}
          </div>
        </div>

        <div style="background:rgba(139,127,240,.08);border:1px solid rgba(139,127,240,.15);
            border-radius:16px;padding:24px;margin-bottom:32px;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
            <div>
              <div style="font-size:14px;font-weight:700;color:#fff;">${plan.name}</div>
              <div style="font-size:11px;color:rgba(255,255,255,.4);">Plan sélectionné</div>
            </div>
            <div style="font-size:20px;font-weight:900;color:var(--ice);">
              ${formatPrice(plan.price, plan.currency)}
            </div>
          </div>
          
          <div style="border-top:1px solid rgba(255,255,255,.1);padding-top:16px;">
            <div style="font-size:11px;color:rgba(255,255,255,.5);margin-bottom:8px;">Inclus :</div>
            <ul style="list-style:none;padding:0;margin:0;font-size:12px;color:rgba(255,255,255,.7);">
              ${(plan.features || []).slice(0, 3).map(f => `
                <li style="padding:4px 0;display:flex;align-items:center;gap:8px;">
                  <span style="color:#10b981;">✓</span>${f}
                </li>`).join('')}
              ${(plan.features || []).length > 3 ? `<li style="padding:4px 0;color:rgba(255,255,255,.4);">+ ${plan.features.length - 3} autres fonctionnalités</li>` : ''}
            </ul>
          </div>
        </div>

        <div style="display:flex;gap:16px;">
          <button id="cancel-payment" style="flex:1;padding:14px;border-radius:12px;border:1px solid rgba(255,255,255,.15);
              background:transparent;color:rgba(255,255,255,.6);font-weight:600;font-size:13px;cursor:pointer;
              transition:all .2s;">
            Annuler
          </button>
          <button id="confirm-payment" style="flex:1;padding:14px;border-radius:12px;border:none;
              background:var(--ice);color:var(--bg);font-weight:800;font-size:13px;cursor:pointer;
              transition:all .2s;">
            Confirmer et payer
          </button>
        </div>

        <div style="margin-top:24px;text-align:center;font-size:10px;color:rgba(255,255,255,.3);
            display:flex;align-items:center;justify-content:center;gap:16px;">
          <span><i class="fa-solid fa-shield-halved"></i> Paiement sécurisé</span>
          <span><i class="fa-solid fa-lock"></i> SSL Chiffré</span>
        </div>
      </div>
    `;
...
// ── Lancer le paiement avec confirmation détaillée ─────────────────
export async function startCheckout(plan) {
  try {
    // Use window.fetchWithAuth since it's a global function
    if (typeof window.fetchWithAuth !== 'function') {
      await import('./utils.js');
    }

    // Confirmation détaillée avant paiement
    const plans = await loadPlans();
    const selectedPlan = plans[plan];
    if (!selectedPlan) {
      showPaymentError('Plan introuvable');
      return;
    }

    const operator = (window.DS_PAYMENT?.selectedOperator || 'notchpay').trim().toLowerCase();
    const operatorName = operator === 'notchpay' ? 'NotchPay' : 'Fapshi';
    
    // Modal de confirmation
    const confirmed = await showPaymentConfirmation(selectedPlan, operatorName);
    if (!confirmed) {
      return;
    }
    ...
    const response = await window.fetchWithAuth(`${API_BASE}/payment/create-checkout`, {
      method: 'POST',
      body: JSON.stringify({
        plan,
        operator,
  ...
  try {
    ...
    return {
      standard: { 
        ...
        price: 25000, 
        currency: 'XAF',
        features: ['Score financier', 'Ratios SYSCOHADA', 'Rapport PDF complet', '3 analyses par mois'] 
      },
      premium:  { 
        name: 'Premium',  
        price: 50000, 
        currency: 'XAF',
        features: ['Toutes les fonctionnalités Standard', 'Chatbot Doctor Smile', 'Simulateur What-If', '10 analyses par mois'] 
      },
      extra:    { 
        name: 'Extra',    
        price: 100000, 
        currency: 'XAF',
        features: ['Toutes les fonctionnalités Premium', 'API REST', 'Support 24/7', 'Analyses illimitées'] 
      },
    };
  }
}
